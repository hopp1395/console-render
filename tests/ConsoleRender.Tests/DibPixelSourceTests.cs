...
public class DibPixelSourceTests
{
        ...
        Func<int, int, (byte B, byte G, byte R)> pixel)
    {
        ...
        for (int row = 0; row < height; row++)
            for (int x = 0; x < width; x++)
            {
                ...
                int offset = 40 + row * stride + x * bytesPerPixel;
                data[offset] = b;
                data[offset + 1] = g;
                data[offset + 2] = r;
            }
    ...
    public void TryParse_ReadsA24BitTopDownImage()
    {
        ...
        byte[] dib = BuildDib(2, 2, 24, bottomUp: false,
    ...
    public void TryParse_FlipsBottomUpImages()
    {
        ...
        byte[] dib = BuildDib(1, 2, 24, bottomUp: true,
    ...
    {
        byte[] dib = BuildDib(3, 1, 32, bottomUp: false, (x, _) => (1, 2, (byte)(x + 3)));
    ...
    public void TryParse_HonoursRowPadding()
    {
        ...
        byte[] dib = BuildDib(3, 2, 24, bottomUp: false, (x, row) => (0, 0, (byte)(row * 3 + x)));
    ...
    {
        byte[] dib = BuildDib(4, 4, 24, bottomUp: false, (_, _) => (0, 0, 0));
    ...
    {
        byte[] dib = BuildDib(2, 2, 24, bottomUp: false, (_, _) => (0, 0, 0));
