...
- In a `switch` statement: a blank line right after the opening `{`, and every `case` block ends with a blank line — including the last one, right before the switch's closing `}`. Exception: a `default` block gets no trailing blank line. Stacked labels (`case A:` / `case B:` sharing one body) count as a single block. Not build-enforced — apply it by hand.
