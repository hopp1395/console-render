...
{
    private readonly Dictionary<KeyCombo, KeyBinding> _bindings = new();

    public IEnumerable<KeyBinding> All => _bindings.Values.OrderBy(b => b.Combo.ToString(), StringComparer.Ordinal);
    ...
    public void Register(KeyCombo combo, string description, Action handler)
    {
        ...
        _bindings[combo] = new KeyBinding(combo, description, handler);
    }

    public void Register(ConsoleKey key, string description, Action handler) =>
        Register(new KeyCombo(key), description, handler);

    public bool Unregister(KeyCombo combo) => _bindings.Remove(combo);
    ...
    {
        if (!_bindings.TryGetValue(KeyCombo.FromKeyInfo(keyInfo), out var binding))
            return false;
        binding.Handler();
        return true;
    }
