...
public sealed class TaskLine
{
    ...
    internal TaskLine(string text) => this.text = text;
    ...
    public void Complete(string? text = null) => Finish('✓', Color.Green, text);

    /// <summary>Freezes the line with a red cross, optionally replacing the text.</summary>
    public void Fail(string? text = null) => Finish('✗', Color.Red, text);
    ...
    internal (string Text, Color Color) Render(double clock) => Running
